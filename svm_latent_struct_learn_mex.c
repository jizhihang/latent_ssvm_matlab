...
//  mxArray const * kernelFn_array;
  ...
  mxGetString(INPUT_STRING, arg, sizeof(arg) / sizeof(char)) ;
  arg_split (arg, &argc, &argv);


  /* read input parameters */
  my_read_input_parameters(argc+1, argv-1, &learn_parm, &kernel_parm, &sparm);

  epsilon = learn_parm.eps;
  C = learn_parm.svm_c;
  MAX_ITER = learn_parm.maxiter;

  /* parse sparm, read in examples*/
  sparm_array = INPUT_SPARM;
  sparm.mex = sparm_array;

  sample = read_struct_examples(sparm_array, &sparm);
...
//  write_struct_model(modelfile, &sm, &sparm);
  ...
  plhs[0] = mxDuplicateArray(model_array) ;
